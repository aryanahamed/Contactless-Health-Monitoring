...
class Stabilization:
    ...
    def __init__(self, mincutoff=0.1, beta=0.2, dcutoff=0.5):
    ...
    def calculate_anchor_velocity(self, current_landmarks, dt):
        ...
        prev_anchors = self.prev_landmarks[self.anchor_indices]
        #displacement anchor
        ...
        #estimate
        alpha_v = 1.0 - np.exp(-2.0 * np.pi * self.dcutoff * dt)
    ...
    def process(self, current_landmarks, timestamp):
        ...
        cutoff = self.mincutoff + self.beta * anchor_velocity
